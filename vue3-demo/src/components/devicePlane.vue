<template>
	<div class="container">
		...
		<!--					&lt;!&ndash;					<img alt="" src="./assets/1ha.png" />&ndash;&gt;-->
		<!--					&lt;!&ndash;					<img alt="" src="./assets/1管理.png" />&ndash;&gt;-->
...
//--------------数据结构转化
let device = $ref([])
const { row_total, col_total } = data.layoutList
//面板结构建立
for (let i = 0; i < row_total; i++) {
	device.push([])
	for (let j = 0; j < col_total; j++) {
		device[i].push([])
	}
}
let showall_param = data.portList.showall_param
const { slot_param, grid_param } = data.layoutList
//接口数据和参数合并
let grid_list = $ref([])
grid_list = grid_param.map((grid) => {
	grid.params = showall_param.filter((show) => {
		return grid.real_name === show.real_name
	})[0]
	return grid
})

for (let i = 0; i < device.length; i++) {
	let len = device[i].length
	for (let j = 0; j < len; j++) {
		slot_param.forEach((slot) => {
			//从接口数组中过滤出当前面板的接口
			let slot_grid_list = grid_list.filter((grid) => {
				return grid.slot_id === slot.slot_id
			})
			...
			let row2 = slot_grid_list.some((v) => {
				return v.row === 2
			})
			if (row2) {
				slot_List.push([[], []])
			...
			//把接口放入面板
			slot_grid_list.forEach((grid) => {
				//c264设备
				slot_List[grid.row][grid.col] = grid
			})
			//c264设备
			if (row2) {
				slot_List[0].unshift([], [])
				slot_List[1].unshift([], [])
				slot_List[1][0] = slot_List[2][0]
				slot_List[1][1] = slot_List[2][1]
				slot_List.pop()
			}
			slot.slot_List = slot_List
			//将slot放入面板
			device[slot.row][slot.col] = slot
		})
	}
}
console.log(device)
