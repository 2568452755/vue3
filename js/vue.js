		...
		for (let i = 0; i < children.length; i++) {
			...
			if ((vIf = findDir(slotElement, 'if'))) {
				...
				if (parseResult) {
					...
					dynamicSlots.push(
						createCallExpression(context.helper(RENDER_LIST), [parseResult.source, createFunctionExpression(createForLoopParams(parseResult), buildDynamicSlot(slotName, slotFunction), true /* force newline */)])
					)
				} else {
